...
import xiatian.chatbot.util.ByteUtil
...
class BaseDb(val dbName: String, val dbPath: File) extends Db {
  ...
  def open(): Unit = {
    LOG.info(s"open DB $dbName with count: ${numKeys()}")
  }

  def numKeys(): Long = db.getLongProperty("rocksdb.estimate-num-keys")

  def close(): Unit = {
    LOG.info(s"===Close DB $dbName === \n\t $dbPath ...")

    cfHandlers.forEach(_.close())
    if (db != null) db.close()
    if (options != null) options.close()

    LOG.warn(s"[DB $dbName CLOSED]\n")
  }
  ...
  def deleteKey(id: String): this.type = {
    db.delete(defaultHandler, id.getBytes(UTF_8))
    this
  }

  def deleteMetaKey(id: String): this.type = {
    db.delete(metaHandler, id.getBytes(UTF_8))
    this
  }
}
